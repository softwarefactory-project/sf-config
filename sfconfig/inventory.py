...
def disable(args, pb):
    host_params = {'action': 'disable', 'erase': False}
    for host in args.inventory:
        host_roles = [role for role in host["roles"] if
                      role not in ("mysql", "install-server")]
        pb.append(host_play(host, host_roles, host_params))
    if 'influxdb' in args.glue['roles']:
        pb.append(host_play('all', 'telegraf', host_params))
    pb.append(host_play('mysql', 'mysql', host_params))
    pb.append(host_play('install-server', 'install-server', host_params))
    pb.append(host_play('all', 'base', host_params))


def erase(args, pb):
    prompt = "WARNING: this playbook will *DESTROY* software factory " \
             "data , press ENTER to continue or CTRL-C to abort"
    host_params = {'action': 'disable', 'erase': True}
...
def run(args):
    ...
    if not args.skip_apply:
        execute(["ansible-playbook", playbook_path])
