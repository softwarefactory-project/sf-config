...
def upgrade(args, pb):
    ...
    pb.append(host_play('localhost', 'repos', {'action': 'upgrade'}))
...
def setup(args, pb):
    ...
    pb.append(host_play('localhost', 'repos', action))
...
def enable_action(args):
    pb = []
    pb.append(host_play('localhost', 'ssh', {'action': 'populate_hosts'}))
    playbook_name = "sfconfig"
    if args.upgrade:
