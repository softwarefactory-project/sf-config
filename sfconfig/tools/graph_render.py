...
def update_cq_action(args):
    ...
    influx_cmd = ['influx', '-ssl', '-host', args.influxdb_host,
                  '-port', '8086',
                  '-username', 'admin', '-password', args.influxdb_password,
                  '-database', 'telegraf', '-execute']
...
#        cmd += ' RESAMPLE FOR %s' % cq_data.get('resample_time', '1h')
...
def single_stat(title, measurement, span=2):
    return {
        'title': title.capitalize(),
        'span': span,
...
def nodepool_dashboard(args):
    ...
    for status in NODE_STATUS:
        nodes_row["panels"].append(single_stat(
            status,
            '/nodepool.provider.*.nodes.%s/' % status
            ))
