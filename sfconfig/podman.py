...
Try it now using:
podman run --systemd --privileged -it --publish-all \
  -v /sys/fs/cgroup:/sys/fs/cgroup:ro --systemd --name my-sf  \
  docker.io/softwarefactoryproject/sf-minimal:latest

Look at port mapping using:
podman port my-sf
...
class Buildah:
    ...
    def system(self):
        ...
        # Add linters tool for local jobs
        self.run("yum install -y yamllint bashate python-flake8 python-tox "
                 "traceroute rpm-build python2-coverage python2-pelican "
                 "python2-sphinx doc8")
    ...
    def sfconfig_install(self):
        ...
        # Add hypervisor-runc
        with open(os.path.join(self.mountpath,
                               "etc/software-factory/arch.yaml"), "a") as of:
            of.write("      - hypervisor-runc\n")
        self.run("sfconfig --skip-setup --enable-insecure-slaves")
