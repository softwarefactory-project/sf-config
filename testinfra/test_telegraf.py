...
class TestTelegraf(utils.Base):
    ...
    def test_telgraft_graphite_output_plugin(self, host):
        if 'graphite-api' in self.enabled_roles():
            graphite_config = '/etc/telegraf/telegraf.d/graphite_output.conf'
            assert host.file(graphite_config).exists
