...
""" % (sfconfig["fqdn"], sfconfig["fqdn"]))
    ...
    if zuul_config.get("external_logserver"):
        for logserver in zuul_config["external_logservers"]:
            server = {
                "name": logserver["name"],
    ...
    if zuul_config.get('log_url'):
        glue["zuul_log_url"] = zuul_config["log_url"]
    else:
        glue["zuul_log_url"] = "%s/logs/{build.parameters[LOG_PATH]}" % \
                               glue["gateway_url"]
