...
# ZuulV3 and NodepoolV3
    ...
    glue["zuul3_success_log_url"] = zuul3_config.get(
        "success_log_url", "%s/logs/{build.uuid}/" % glue["gateway_url"]
    )
    glue["zuul3_failure_log_url"] = zuul3_config.get(
        "failure_log_url", "%s/logs/{build.uuid}/" % glue["gateway_url"]
    )
